...
class OrderTest(TestCase):
    def setUp(self):
        ...
        ProductVariant.objects.create(name='коробка 10x10x10 черная', addition='черная', slug='box10black', product=Product.objects.get(pk=1), price=Decimal('10'))
        ProductVariant.objects.create(name='шнур 10см жёлтый', addition='жёлтый', slug='cord10yellow', product=Product.objects.get(pk=2), price=Decimal('1'))
        
        Organisation.objects.create(inn='1234567890')
        User.objects.create(username='1@test.com', email='1@test.com', password='password')
        User.objects.create(username='2@test.com', email='2@test.com', password='password')
        Group.objects.create(name='const customers')
        User.objects.get(pk=1).groups.add(Group.objects.get(pk=1))

        Order.objects.create(organisation=Organisation.objects.get(pk=1), user=User.objects.get(pk=1))
        OrderItem.objects.create(product=ProductVariant.objects.get(pk=1), price=ProductVariant.objects.get(pk=1).price)
